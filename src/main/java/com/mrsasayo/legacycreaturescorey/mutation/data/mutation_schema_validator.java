...
/**
 * Validador de esquema JSON para mutaciones antes de parsearlas.
 */
final class mutation_schema_validator {
    static final mutation_schema_validator INSTANCE = new mutation_schema_validator();

    private static final Map<String, ActionValidator> ACTION_VALIDATORS = createValidators();

    private mutation_schema_validator() {
    }
    ...
    private static Map<String, ActionValidator> createValidators() {
        ...
        register(builder, seenKeys, mutation_schema_validator::validateAttributeAction, "attribute", "attribute_modifier");
        register(builder, seenKeys, mutation_schema_validator::validateStatusEffectOnHitAction, "status_effect_on_hit", "status_effect");
        register(builder, seenKeys, mutation_schema_validator::validateStatusEffectOnDeathAction, "status_effect_on_death", "on_death_status_effect", "status_effect_death");
        register(builder, seenKeys, mutation_schema_validator::validateGroundHazardOnDeathAction, "ground_hazard_on_death", "ground_hazard");
        register(builder, seenKeys, mutation_schema_validator::validateDamageAuraAction, "damage_aura", "aura_damage");
        register(builder, seenKeys, mutation_schema_validator::validateHealAction, "heal", "heal_self");
        register(builder, seenKeys, mutation_schema_validator::validateSummonAction, "summon_mob", "summon");
        register(builder, seenKeys, mutation_schema_validator::validateStatusEffectAuraAction, "status_effect_aura", "aura_status_effect");
        register(builder, seenKeys, mutation_schema_validator::validateCriticalAction, "critical_hit", "critical_damage");
        register(builder, seenKeys, mutation_schema_validator::validateHealOnHitAction, "heal_on_hit", "lifesteal");
        register(builder, seenKeys, mutation_schema_validator::validateDamageArmorAction, "damage_armor", "anti_armor");
        register(builder, seenKeys, mutation_schema_validator::validateIgniteAction, "ignite", "ignite_on_hit", "set_on_fire");
        register(builder, seenKeys, mutation_schema_validator::validateBleedingAction, "bleed", "bleeding");
        register(builder, seenKeys, mutation_schema_validator::validateKnockbackAction, "knockback", "push");
        register(builder, seenKeys, mutation_schema_validator::validateTeleportAction, "teleport", "teleport_on_hit", "teleportation");
        register(builder, seenKeys, mutation_schema_validator::validateExperienceTheftAction, "experience_theft", "xp_theft");
        register(builder, seenKeys, mutation_schema_validator::validateVerticalThrustAction, "vertical_thrust", "launch");
        register(builder, seenKeys, mutation_schema_validator::validateShatterArmorAction, "shatter_armor", "armor_shatter");
        register(builder, seenKeys, mutation_schema_validator::validateProjectileShroudAction, "projectile_shroud");
        register(builder, seenKeys, mutation_schema_validator::validateInterferenceAuraAction, "interference_aura", "aura_interference");
        register(builder, seenKeys, mutation_schema_validator::validateStasisFieldAction, "stasis_field_aura", "aura_stasis", "stasis_field");
        register(builder, seenKeys, mutation_schema_validator::validateEntropyAuraAction, "entropy_aura", "aura_entropy", "entropy");
        register(builder, seenKeys, mutation_schema_validator::validatePsionicThornsAction, "psionic_thorns", "psionic_thorns_aura", "thorns_aura");
        register(builder, seenKeys, mutation_schema_validator::validateDeepDarknessAction, "deep_darkness_aura", "darkness_aura", "deep_darkness");
        register(builder, seenKeys, mutation_schema_validator::validateVirulentGrowthAction, "virulent_growth_aura", "growth_aura", "virulent_growth");
        register(builder, seenKeys, mutation_schema_validator::validateHordeBeaconAction, "horde_beacon_aura", "horde_beacon");
        register(builder, seenKeys, mutation_schema_validator::validateUndeadPotionBurstAction, "undead_potion_burst", "potion_burst");
        register(builder, seenKeys, mutation_schema_validator::validateAttributeAuraAction, "attribute_aura");
        register(builder, seenKeys, mutation_schema_validator::validateAllyDeathHealAction, "ally_death_heal", "ally_death_aura");
        register(builder, seenKeys, mutation_schema_validator::validateChaosTouchAction, "chaos_touch");
        register(builder, seenKeys, mutation_schema_validator::validateFrenzyAction, "frenzy");
        register(builder, seenKeys, mutation_schema_validator::validatePainLinkAction, "pain_link");
        register(builder, seenKeys, mutation_schema_validator::validateEssenceSiphonAction, "essence_siphon");
        register(builder, seenKeys, mutation_schema_validator::validateConcussiveBlowAction, "concussive_blow");
        register(builder, seenKeys, mutation_schema_validator::validateMortalWoundAction, "mortal_wound");
        register(builder, seenKeys, mutation_schema_validator::validateDisarmAction, "disarm", "disarm_on_hit");
        register(builder, seenKeys, mutation_schema_validator::validateFreezeAction, "freeze", "freeze_on_hit");
        register(builder, seenKeys, mutation_schema_validator::validateDisableShieldAction, "disable_shield", "anti_shield");
        register(builder, seenKeys, mutation_schema_validator::validateTrueDamageAction, "true_damage", "true_damage_on_hit");
        register(builder, seenKeys, mutation_schema_validator::validatePhantasmalVeilAuraAction, "phantasmal_veil", "phantasmal_veil_aura", "aura_phantasmal");
    ...
    private static void put(ImmutableMap.Builder<String, ActionValidator> builder,
            ...
            ActionValidator validator) {
        if (!seenKeys.add(key)) {
            Legacycreaturescorey.LOGGER.warn("⚠️ Acción duplicada '{}' en mutation_schema_validator. Se ignorará el alias repetido.", key);
            return;
        }
    ...
    private static void validateDurationField(JsonObject obj, ValidationCollector collector, String path) {
    ...
    private static void requireString(JsonObject object, String key, ValidationCollector collector, String path, boolean required) {
    ...
    private static void requireInteger(JsonObject object, String key, ValidationCollector collector, String path, boolean required, int min) {
        if (!object.has(key) || object.get(key).isJsonNull()) {
            if (required) {
                collector.error(path, "Campo obligatorio ausente");
            }
    ...
    private static void requireNumber(JsonObject object, String key, ValidationCollector collector, String path, boolean required) {
        if (!object.has(key) || object.get(key).isJsonNull()) {
            if (required) {
                collector.error(path, "Campo obligatorio ausente");
            }
    ...
    private static void requireIdentifier(JsonObject object, String key, ValidationCollector collector, String path, boolean required) {
        if (!object.has(key) || object.get(key).isJsonNull()) {
            if (required) {
                collector.error(path, "Campo obligatorio ausente");
            }
    ...
    private static void requireArray(JsonObject object, String key, ValidationCollector collector, String path, boolean required) {
        if (!object.has(key) || object.get(key).isJsonNull()) {
            if (required) {
                collector.error(path, "Campo obligatorio ausente");
            }
