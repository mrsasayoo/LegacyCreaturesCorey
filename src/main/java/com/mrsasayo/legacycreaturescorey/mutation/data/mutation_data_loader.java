...
import com.mrsasayo.legacycreaturescorey.core.api.event.MutationDataEvents;
...
import com.mrsasayo.legacycreaturescorey.mutation.a_system.incompatibility_manager;
...
import com.mrsasayo.legacycreaturescorey.mutation.action.helper.*;
...
public final class mutation_data_loader implements SimpleSynchronousResourceReloadListener {
    ...
    private static final mutation_schema_validator SCHEMA_VALIDATOR = mutation_schema_validator.INSTANCE;

    private mutation_data_loader() {
    }

    public static void register() {
        if (REGISTERED.compareAndSet(false, true)) {
            ResourceManagerHelper.get(ResourceType.SERVER_DATA).registerReloadListener(new mutation_data_loader());
        }
    }
    ...
    public void reload(ResourceManager manager) {
        ...
        for (Map.Entry<Identifier, Resource> entry : resources.entrySet()) {
            ...
            try {
                ...
                try (InputStream stream = resource.getInputStream();
                    ...
                    try {
                        mutation m = parseMutation(mutationId, root, actionParser);
                        if (m != null) {
                            parsed.add(m);
                        }
        ...
        mutation_registry.registerDynamicMutations(parsed);
    ...
    private mutation parseMutation(Identifier fileId, JsonObject root, ActionParser actionParser)
        ...
        mutation_type type = resolveMutationType(rawType);
        ...
        incompatibility_manager restrictions = parseRestrictions(root);

        List<mutation_action> actions = new ArrayList<>();
        ...
        mutation_action specificAction = loadSpecificAction(id, specificConfig);
        ...
        return new configured_mutation(id, type, cost, displayName, description, weight, actions, incompatibleWith,
                restrictions);
    }

    private mutation_type resolveMutationType(String rawType) {
        if (rawType == null || rawType.isBlank()) {
            throw new IllegalArgumentException("El campo 'type' es obligatorio para las mutaciones.");
        }
        String normalized = rawType.trim().toUpperCase(Locale.ROOT);
        return switch (normalized) {
            case "AURA", "AURAS", "ACTIVE_AURA" -> mutation_type.AURAS;
            case "PASSIVE_ATTRIBUTE", "ATTRIBUTE" -> mutation_type.PASSIVE_ATTRIBUTE;
            case "PASSIVE" -> mutation_type.PASSIVE;
            case "ON_HIT", "ONHIT" -> mutation_type.ON_HIT;
            case "PASSIVE_ON_HIT" -> mutation_type.PASSIVE_ON_HIT;
            case "MOB_EXCLUSIVE" -> mutation_type.MOB_EXCLUSIVE;
            case "ON_BEING_HIT" -> mutation_type.ON_BEING_HIT;
            case "ON_DEATH" -> mutation_type.ON_DEATH;
            case "SYNERGY" -> mutation_type.SYNERGY;
            case "TERRAIN" -> mutation_type.TERRAIN;
            default -> mutation_type.valueOf(normalized);
        };
    }
    ...
    private mutation_action loadSpecificAction(Identifier id, JsonObject configObject) {
    ...
    private incompatibility_manager parseRestrictions(JsonObject root) {
        if (!root.has("restrictions")) {
            return new incompatibility_manager(Set.of(), Set.of(), Set.of(), Set.of(), false);
        }
        ...
        return new incompatibility_manager(
                allowed.ids(),
                excluded.ids(),
                allowed.tags(),
                excluded.tags(),
    ...
    static final class ActionParser {
        ...
        List<mutation_action> parseActions(Identifier mutationId, JsonArray array) throws ParseException {
            if (array == null || array.isEmpty()) {
                return List.of();
            }
            List<mutation_action> actions = new ArrayList<>(array.size());
            for (int i = 0; i < array.size(); i++) {
                ...
                mutation_action action = switch (normalized) {
        ...
        private mutation_action parseAttributeAction(JsonObject root) {
            ...
            attribute_mutation_action.Mode mode = attribute_mutation_action.Mode.valueOf(modeStr.toUpperCase(Locale.ROOT));
            return new attribute_mutation_action(Identifier.of(attributeId), mode, amount);
        }

        private mutation_action parseHealAction(JsonObject root) {
        ...
        private mutation_action parseAllyDeathHealAuraAction(JsonObject root) {
