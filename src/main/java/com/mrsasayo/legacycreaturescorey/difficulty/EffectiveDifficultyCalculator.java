...
public final class EffectiveDifficultyCalculator {
    ...
    double biomeMultiplier = boostedBiome ? CoreyConfig.INSTANCE.biomeDifficultyMultiplier : 1.0D;
    double antiFarmMultiplier = resolveAntiFarmMultiplier(state, origin, config);
    ...
    private static double resolveAntiFarmMultiplier(CoreyServerState state, BlockPos origin, CoreyConfig config) {
        if (!config.antiFarmDetectionEnabled || !config.antiFarmHeatPenaltyEnabled) {
            return 1.0D;
        }
        ...
        double minMultiplier = clamp01(config.antiFarmHeatPenaltyMinMultiplier);
        ...
        int threshold = Math.max(1, config.antiFarmKillThreshold);
        double ratio = Math.min(1.0D, (double) killCount / threshold);
        double exponent = Math.max(0.1D, config.antiFarmHeatPenaltyExponent);
