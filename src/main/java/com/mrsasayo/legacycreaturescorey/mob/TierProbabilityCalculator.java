...
public final class TierProbabilityCalculator {
    ...
    public static MobTier chooseTier(
        ...
        boolean verbose = config.debugLogProbabilityDetails;
    ...
    private static double getMultiplier(MobTier tier) {
        CoreyConfig config = CoreyConfig.INSTANCE;
        return switch (tier) {
            case EPIC -> config.epicChanceMultiplier;
            case LEGENDARY -> config.legendaryChanceMultiplier;
            case MYTHIC -> config.mythicChanceMultiplier;
            case DEFINITIVE -> config.definitiveChanceMultiplier;
            default -> 1.0D;
        };
    }
