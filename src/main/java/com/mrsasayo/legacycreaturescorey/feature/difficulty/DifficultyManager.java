...
public class DifficultyManager {
    ...
    public static void checkDailyIncrease(MinecraftServer server) {
        ...
        if (currentDay > lastDay) {
            ...
            state.decayChunkHeat(daysPassed, antifarm_config.getDailyDecayAmount());

            int oldDifficulty = state.getGlobalDifficulty();
            int targetDifficulty = (int) Math.min(difficulty_config.getMaxGlobalDifficulty(), currentDay);
    ...
    private static void triggerPlayerDifficultyRolls(MinecraftServer server, CoreyServerState state, long daysPassed) {
        ...
        double chance = difficulty_config.getPlayerDifficultyIncreaseChance();
        if (chance <= 0.0) {
            return;
        }
    ...
    public static void applyDeathPenalty(ServerPlayerEntity player) {
        ...
        long streakWindow = Math.max(0L, difficulty_config.getDeathPenaltyCooldownTicks());
        ...
        int basePenalty = Math.max(0, difficulty_config.getDeathPenaltyAmount());
        if (basePenalty <= 0) {
    ...
    private static void sendDifficultySnapshot(ServerPlayerEntity player, CoreyServerState state, PlayerDifficultyData data) {
        DifficultySyncPayload payload = new DifficultySyncPayload(
            state.getGlobalDifficulty(),
            difficulty_config.getMaxGlobalDifficulty(),
            data.getPlayerDifficulty(),
            difficulty_config.isEnableDifficultyHud() && data.isDifficultyHudEnabled()
        );
